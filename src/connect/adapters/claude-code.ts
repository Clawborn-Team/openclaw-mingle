 ...
 */
export function createClaudeCodeAdapter(opts: { run?: CommandRunner; bin?: string } = {}): RuntimeAdapter {
  ...
  return {
    ...
    async respond(turn: RuntimeTurn): Promise<string> {
      ...
      if (turn.dir) args.push("--add-dir", turn.dir);
      if (turn.model) args.push("--model", turn.model);
      const res = await run(bin, args, { timeoutMs: 120_000 });
      const out = res.stdout.trim();
      if (!out) throw new Error(`claude-code produced no output (exit ${res.code}): ${res.stderr.slice(0, 200)}`);
      return out;
    },
  };
}
