...
const POSITIVE_LIMIT = { type: "integer", minimum: 1, maximum: 100 };
const STRING_LIST = {
    type: "array",
    items: { type: "string" },
    maxItems: 6,
...
function optionalInteger(params, name, options = {}) {
    const value = params[name];
    if (value === undefined)
        return undefined;
...
function optionalStringList(params, name) {
    const value = params[name];
    if (value === undefined)
        return undefined;
...
function tool(name, label, description, parameters, execute) {
    return {
        ...
        async execute(_toolCallId, params) {
            return jsonResult(await execute(params));
        },
    };
}
export function createMingleTools(params) {
    ...
    return [
        ...
        tool("mingle_read_channel", "Read Mingle Channel", "Read messages from a Mingle group, event, or plaza channel by slug.", objectSchema({
            slug: STRING,
        ...
        tool("mingle_list_introductions", "List Mingle Introductions", "List introduction proposals visible to the authenticated Mingle account.", EMPTY_SCHEMA, async () => client.listIntroductions()),
        tool("mingle_respond_introduction", "Respond to Mingle Introduction", "Accept or decline an introduction proposal only when the human's intent is clear.", objectSchema({ id: STRING, action: { type: "string", enum: ["accept", "decline"] } }, ["id", "action"]), async (input) => {
        ...
        tool("mingle_get_profile", "Get Mingle Profile", "Read the authenticated account's own Mingle profile and passport settings.", EMPTY_SCHEMA, async () => client.getProfile()),
        tool("mingle_update_profile", "Update Mingle Profile", "Update the authenticated account's Mingle profile after confirming profile details with its human owner.", objectSchema({
            display_name: STRING,
