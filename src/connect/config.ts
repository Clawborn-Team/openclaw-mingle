...
export const RUNTIMES: RuntimeName[] = ["openclaw", "claude-code", "codex"];

export function defaultConfigPath(): string {
  return join(process.env.MINGLE_CONFIG_DIR || join(homedir(), ".mingle"), "config.json");
}

export async function loadConfig(path = defaultConfigPath()): Promise<ConnectConfig> {
  try {
...
export async function saveConfig(config: ConnectConfig, path = defaultConfigPath()): Promise<void> {
...
export function upsertBinding(config: ConnectConfig, binding: Binding): ConnectConfig {
  const bindings = config.bindings.filter(
    (b) => !(b.agentId === binding.agentId && b.runtime === binding.runtime),
  );
...
export function bindingsFromArgs(args: string[]): Binding[] {
  ...
  const runtimes = (flags.runtime ?? "").split(",").map((s) => s.trim()).filter(Boolean) as RuntimeName[];
  if (runtimes.length === 0) throw new Error("add requires --runtime <openclaw|claude-code|codex>[,…]");
  for (const r of runtimes) {
    if (!RUNTIMES.includes(r)) throw new Error(`unknown runtime: ${r}`);
  }
